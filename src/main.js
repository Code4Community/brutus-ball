...
class Game {
  ...
  processNextTurn() {
    if (processTurn(game.players[game.nextTurn])) {
      game.didSomethingThisIteration = true;
    }
    game.nextTurn = (game.nextTurn + 1);
    if (game.nextTurn >= game.players.length) {
      game.nextTurn = 0;
      game.roundID++;
      if (!game.didSomethingThisIteration) {
        console.log("---- END");
        game.manualStop();
      }
      game.didSomethingThisIteration = false;
    }
  }
  ...
  checkEndgame() {
    ...
    if (winner == -1) {
      ...
      game.logStr += "PLAYER " + (winner + 1) + " WON\n";
      game.logStr += "\n------\n";
  ...
  manualStop() {
    window.clearInterval(game.turnTimer);
    game.turnTimer = null;
    game.didSomethingThisIteration = false;
    game.roundID = 1;
    game.eventCount = 1;
  ...
  addEvent(player, imgHTML, textHTML) {
...
function collisionHandler(obj1, obj2) {
  ...
  if (obj2.c4cPlayer != obj1.c4cSource) {
    ...
    game.log(obj2.c4cPlayer, "was hit");
    game.addEvent(obj2.c4cPlayer.name, "Hit!", `${obj2.c4cPlayer.name} was hit!`);
    game.updateWinIndicator();
...
export { makeGame, runSimulation, game };
